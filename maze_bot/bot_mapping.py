...
class bot_mapper():
    ...
    def display_connected_nodes(self, curr_node, neighbor_node, case="Unkown", color=(0, 0, 255)):
        ...
        self.maze_connect = cv2.line(
            self.maze_connect, curr_pixel, neighbor_pixel, color, 5)
        ...
        # cv2.waitKey(0)
        self.maze_connect = cv2.line(
            self.maze_connect, curr_pixel, neighbor_pixel, (255, 255, 255), 1)
        ...
        if (maze[node_row-step_up][node_col-step_l] > 0):
            ...
            if neighbor_node in self.Graph.graph.keys():
                ...
                if not self.connected_left:
                    self.display_connected_nodes(
                        curr_node, neighbor_node, "LEFT", (0, 0, 255))
                    ...
                    self.connect_neighbors(
                        maze, node_row, node_col, case, step_l, step_up, totl_cnncted)
                if not self.connected_upleft:
                    self.display_connected_nodes(
                        curr_node, neighbor_node, "UPLEFT", (0, 128, 255))
                ...
                if not self.connected_up:
                    self.display_connected_nodes(
                        curr_node, neighbor_node, "UP", (0, 255, 0))
                ...
                if not self.connected_upright:
                    self.display_connected_nodes(
                        curr_node, neighbor_node, "UPRIGHT", (255, 0, 0))
    ...
    def one_pass(self, maze):  # Extract interest points from the maze
        ...
        for row in range(rows):
            for col in range(cols):
                if maze[row, col] == 255:
                    if debug_mapping:
                        self.maze_connect = cv2.cvtColor(
                            maze, cv2.COLOR_GRAY2BGR)

                    # Probable IP =  Find surrounding pixels intensities
                    top_left, top, top_rgt, rgt, btm_rgt, btm, btm_left, lft, paths = self.get_surround_pixel_intensities(
                        maze.copy(), row, col)
    ...
    def graphify(self, extracted_maze):  # Extracted maze is received from the localization stage
        ...
        if not self.graphified:
            ...
            thinned_dilated = cv2.morphologyEx(
                thinned, cv2.MORPH_DILATE, kernel)
            _, bw2 = cv2.threshold(
